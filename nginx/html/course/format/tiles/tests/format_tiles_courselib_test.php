...
class format_tiles_course_courselib_testcase extends advanced_testcase {
    ...
    public function test_course_get_recent_courses() {
        ...
        $courses = array();
        for ($i = 1; $i < 4; $i++) {
            $courses[]  = $generator->create_course(array('format' => 'tiles'));
        };
    ...
    public function test_course_allowed_module() {
        ...
        $this->assertTrue(
            $CFG->version < 2020050000 ? true // Ignore this test in Moodle 3.8 and earlier
                : course_allowed_module($course, 'assign', $manager)
        );
    }
}
